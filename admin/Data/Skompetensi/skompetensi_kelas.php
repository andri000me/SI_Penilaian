...
$kode2=$data['kompetensi_kode'];?>
     <td><?php $sql0=mysql_query("select tb_standarkompetensi.* from tb_standarkompetensi where tb_standarkompetensi.kompetensi_kode='$kode2'") or die (mysql_query()); $cek=mysql_num_rows($sql0); if($cek>0){ ?>
	        <div class="btn-group">
	            <a href="?page=skompetensiview&kompetensi=<?php echo $data['kompetensi_kode']; ?>&kelas=<?php echo $data['kelas']; ?>"><button class='btn btn-mini btn-primary'> Lihat</button></a>
	        </div><?php }else{ ?>
			<div class="btn-group">
	            <a href="?page=skompetensiview&kompetensi=<?php echo $data['kompetensi_kode']; ?>&kelas=<?php echo $data['kelas']; ?>"><button class='btn btn-mini btn-danger'> Lihat</button></a>
	        </div><?php } ?><!-- /btn-group -->
		</td> </tr></tbody>
        <?php
$i;
