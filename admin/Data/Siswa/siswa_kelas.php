...
$kode2=$data['kode_kelas'];?>
     <td><?php $sql0=mysql_query("select tb_siswa.* from tb_siswa where tb_siswa.kode_kelas='$kode2'"); $cek=mysql_num_rows($sql0); if($cek>0){ ?>
	        <div class="btn-group">
	            <a href="?page=siswaview&kelas=<?php echo $data['kode_kelas']; ?>"><button class='btn btn-mini btn-primary'> Lihat</button></a>
	        </div><?php }else{ ?>
			<div class="btn-group">
	           <a href="?page=siswaview&kelas=<?php echo $data['kode_kelas']; ?>"> <button class='btn btn-mini btn-danger'> Belum Ada</button></a>
	        </div><?php } ?><!-- /btn-group -->
		</td> </tr></tbody>
        <?php
$i;
