...
<?php $kelas=$_GET['kelas'];$nisn=$_GET['nisn']; $h=mysql_query("select * from tb_kelas where kode_kelas='$kelas'"); $fg=mysql_fetch_array($h);?>
<div class="table-header">
  <div class="nav-collapse"> <strong>Raport <?php  echo $fg['nama_kelas']?>  &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</strong> <a target="_blank" href="Data/laporan_nilai/laporan_nilaicetak.php?kelas=<?php echo $kelas ?>&nisn=<?php echo $nisn ?>">
    <button type="submit" class="btn btn-inverse" name="btncari"><i class="icon-print"></i>Cetak</button>
  </a> &nbsp;&nbsp;&nbsp;
  <!-----------------word------------->
  <a target="_blank" href="Data/laporan_nilai/laporan_nilaiword.php?kelas=<?php echo $kelas ?>&nisn=<?php echo $nisn ?>">
    <button type="submit" class="btn btn-info" name="btncari"><i class="icon-save"></i>Word</button>
  </a>&nbsp;&nbsp;&nbsp;
  <!-----------------excel------------->
  <a target="_blank" href="Data/laporan_nilai/laporan_nilaiexcel.php?kelas=<?php echo $kelas ?>&nisn=<?php echo $nisn ?>">
    <button type="submit" class="btn btn-success" name="btncari"><i class="icon-save"></i>Excel</button>
  </a>&nbsp;&nbsp;&nbsp;
  <!-----------------pdf------------->
  <a target="_blank" href="Data/laporan_nilai/laporan_nilaipdf.php?kelas=<?php echo $kelas ?>&nisn=<?php echo $nisn ?>">
  ...
  <tr>
    <td colspan="6"><table width="751" border="0" class="table table-bordered table-hover">
      ...
      <tr>
        ...
        <td><?php if($uj['nilai_angka']<$tg['kkm']){ ?> <font color="#FF0000"> <?php 

		 if(isset($uj['nilai_angka'])){ echo $uj['nilai_angka']; }else{ echo "0"; } ?></font>
		 <?php }else{ if(isset($uj['nilai_angka'])){ echo $uj['nilai_angka']; }else{ echo "0"; } } ?>
		 </td>
